...
class VisionLlmEvidenceExtractor(VisualEvidenceExtractorPort):
    ...
    def extract_visual(
        ...
        if data_url is None:
            return EvidenceExtractionResult(
                ...
                items=(),
                metadata={"source": "vision_llm", "status": "no_visual_input"},
            )

        response = self._llm_runtime.invoke(
            LlmRuntimeRequest(
                operation_name="evidence_extraction_visual",
                messages=(
                    {
                        "role": "system",
                        "content": (
                            "You are a visual evidence extractor. "
                            "You must faithfully transcribe and summarize the visible content. "
                            "Return plain text useful for evidence, without inventing content."
                        ),
                    },
                    {
                        "role": "user",
                        "content": [
                            {
                                "type": "text",
                                "text": (
                                    "Extract the relevant visible text and content from this image. "
                                    "Prioritize legible text, titles, dates, documentary names, and structural signals."
                                ),
                            },
                            {
                                "type": "image_url",
                ...
                metadata={
                    **dict(request.metadata),
                    "evidence_extraction_kind": "visual",
                },
            )
        )
        ...
        if not text:
            return EvidenceExtractionResult(
                carrier=request.artifact.infer_carrier(),
                items=(),
                metadata={"source": "vision_llm", "status": "empty_response"},
            )
        ...
        return EvidenceExtractionResult(
            ...
            items=(
                EvidenceItem(
                    ...
                    carrier=carrier,
                    text=text,
                    metadata={"extracted_by": "vision_llm"},
                ),
            ),
            metadata={"source": "vision_llm"},
        )
